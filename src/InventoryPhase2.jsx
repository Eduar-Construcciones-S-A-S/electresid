...
export default function InventoryPhase2({profile}){
  ...
  const run=async(type)=>{
    ...
    if(type==='entrada'){
      ...
      res=await supabase.rpc('registrar_entrada_inventario',{p_variante_id:product,p_cantidad:n,p_observacion:note||null,p_costo_unitario:cost?Number(cost):null})
    }else if(type==='salida'){
  ...
  return <div className="inv2">
    ...
    <section className="panel inv2-shell">
      ...
      {tab==='entrada'&&<MovementForm type="entrada" stock={stock} product={product} setProduct={setProduct} qty={qty} setQty={setQty} cost={cost} setCost={setCost} note={note} setNote={setNote} selected={selected} canAdjust={canAdjust} run={run} reset={reset}/>} 
...
function InventoryCurrent({rows,loading,query,setQuery}){
  return <div><div className="inv2-toolbar"><div className="search"><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar producto, SKU, categoría o marca..."/></div></div><div className="table-wrap"><table><thead><tr><th>Producto</th><th>SKU</th><th>Categoría</th><th>Stock actual</th><th>Stock mínimo</th><th>Estado</th><th>Costo</th><th>Valor inventario</th></tr></thead><tbody>{loading?<tr><td colSpan="8" className="empty">Cargando inventario...</td></tr>:rows.length?rows.map(x=><tr key={x.variante_id}><td><b>{x.producto}</b><small className="cell-sub">{x.variante||'Sin variante'}</small></td><td>{x.sku}</td><td>{x.categoria}</td><td><strong>{num(x.stock_actual)}</strong></td><td>{num(x.stock_minimo)}</td><td><StockBadge state={x.estado_stock}/></td><td>{money(x.costo)}</td><td>{money(x.valor_costo)}</td></tr>):<tr><td colSpan="8" className="empty">No hay productos con inventario.</td></tr>}</tbody></table></div></div>
}
