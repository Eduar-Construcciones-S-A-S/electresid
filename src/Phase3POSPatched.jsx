...
function FixedPOS(){
  ...
  const add=x=>{const stock=Number(x.stock_actual||0);if(stock<=0){setMsg('Este producto está agotado. Resúrtelo antes de vender.');return}setCart(c=>{const old=c.find(y=>y.id===x.id);const next=(old?.qty||0)+1;if(next>stock){setMsg('No hay más unidades disponibles de este producto.');return c}setMsg('');return old?c.map(y=>y.id===x.id?{...y,qty:next}:y):[...c,{...x,qty:1}]})}
  ...
  const sell=async()=>{
    ...
    const{data,error}=await supabase.rpc('registrar_venta',{p_items:cart.map(x=>({variante_id:x.id,cantidad:x.qty})),p_pagos:payload,p_cliente_id:client||null,p_caja_id:cash,p_descuento:Number(discount||0),p_observacion:null})
